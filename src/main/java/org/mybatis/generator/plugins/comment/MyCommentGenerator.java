    ...
    public void addModelClassComment(TopLevelClass clazz, IntrospectedTable table) {
        ...
        if (addRemarkComments && StringUtility.stringHasValue(remarks)) {
            ...
            for (String remarkLine : remarkLines) {
                clazz.addJavaDocLine(" *   " + remarkLine); //$NON-NLS-1$
            }
        }
        clazz.addJavaDocLine(" * This class corresponds to the database table <strong>"
                + table.getFullyQualifiedTable() + "</strong>");
        List<IntrospectedColumn> columns = table.getAllColumns();
        if (columns != null && !columns.isEmpty()) {
            clazz.addJavaDocLine(" * ");
            clazz.addJavaDocLine(" * <pre>");
            for (IntrospectedColumn column : columns) {
                clazz.addJavaDocLine(" *  " + column.getJavaProperty() + "\t"
                        + column.getFullyQualifiedJavaType().getShortName() + "\t" + remarks);
            }
            clazz.addJavaDocLine(" * </pre>");
        }
        clazz.addJavaDocLine(" */"); //$NON-NLS-1$
